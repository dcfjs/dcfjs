...
import { BufferList, EncodingType, EncodeOptions, Encoder } from './type';
...
export function buildEncode(
  ...
  function encode(obj: unknown): Buffer | BufferList {
    if (obj === undefined) return Buffer.from([0xc1]);
    ...
    if (obj && (Buffer.isBuffer(obj) || obj instanceof Uint8Array)) {
      if (obj instanceof Uint8Array) {
        obj = Buffer.from(obj);
      }
      // weird hack to support Buffer
      // and Buffer-like objects
      return bl([getBufferHeader((obj as Buffer).length), obj]);
    }
    if (Array.isArray(obj)) return encodeArray(obj, encode);
    if (typeof obj === 'object')
      return (
        encodeExt(obj, encodingTypes) ||
        encodeObject(obj as Record<string, unknown>, options, encode)
      );
...
function encodeArray(array: unknown[], encode: Encoder) {
  const acc: Array<Buffer | BufferList> = [getHeader(array.length, 0x90, 0xdc)];
...
function encodeMap(
  ...
  const acc: Array<Buffer | BufferList> = [getHeader(map.size, 0x80, 0xde)];
  const keys = [...map.keys()];

  if (!options.preferMap) {
    if (keys.every((item) => typeof item === 'string')) {
      console.warn(
        'Map with string only keys will be deserialized as an object!'
      );
    }
  }
...
function encodeObject(
  ...
  const acc: Array<Buffer | BufferList> = [getHeader(keys.length, 0x80, 0xde)];
...
function write64BitUint(buf: Buffer, offset: number, num: number) {
...
function negate64BitInt(buf: Buffer, offset: number) {
  let i = offset + 8;
...
function encodeFloat(obj: number, forceFloat64?: boolean) {
  let buf;
...
function encodeExt(obj: unknown, encodingTypes: EncodingType[]) {
...
function getExtHeader(length: number) {
  ...
  if (length < 0x10000)
    return Buffer.from([0xc8, length >> 8, length & 0x00ff]);
  return Buffer.from([
    0xc9,
    length >> 24,
    (length >> 16) & 0x000000ff,
    (length >> 8) & 0x000000ff,
    length & 0x000000ff,
  ]);
}

function getHeader(length: number, tag1: number, tag2: number) {
...
function encodeString(obj: string, options: EncodeOptions) {
...
function getBufferHeader(length: number) {
  let header;
  if (length <= 0xff) {
...
function encodeNumber(obj: number, options: EncodeOptions) {
  let buf: Buffer;
